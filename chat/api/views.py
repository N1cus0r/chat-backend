...
from .serializers import (RoomSerializer, CreateRoomSerializer,
                          MyTokenObtainSerializer, UserSerializer,
                          MessageSerializer)
...
class JoinRoom(APIView):
    ...
    def put(self, request):
        ...
        if code is not None:
            ...
            return Response({'error': 'Room not found, invalid room code'}, status=status.HTTP_404_NOT_FOUND)
        return Response({'error': 'Room code was not provided in the body'}, status=status.HTTP_404_NOT_FOUND)
    ...
    def get(self, request):
        ...
        if room_id is not None:
            messages = Message.objects.filter(
                room_id=room_id).order_by('-time_sent')[:10]
